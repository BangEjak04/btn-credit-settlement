...
class UserSettlementTable extends TableWidget
{
    protected int|string|array $columnSpan = 'full';

    protected function getTableHeading(): string|Htmlable|null
    {
        return __('filament::widget.heading.settlement');
    }

    protected function getTableDescription(): string|Htmlable|null
    {
        return __('filament::widget.description.settlement');
    }

    public function getTableModelLabel(): ?string
    {
        return __('filament::widget.model.label.settlement');
    }

    public function table(Table $table): Table
    {
        return $table
            ->query(fn (): Builder => Settlement::where('user_id', Filament::auth()->id()))
            ->columns([
                TextColumn::make('status')
                    ->label(__('filament::widget.form.label.status'))
                    ->formatStateUsing(fn ($record) => Str::headline($record->status))
                    ->badge(),
                TextColumn::make('property_address')
                    ->label(__('filament::widget.form.label.property_address')),
                TextColumn::make('updated_at')
                    ->label(__('filament::widget.form.label.updated_at'))
                    ->since(),
            ])
            ->filters([
                //
            ...
            ->headerActions([
                CreateAction::make()
                    ->schema([
                        View::make('filament.widgets.brochure-offer')
                            ->hidden(fn (Get $get): bool => $get('have_accepted_offer') === true)
                            ->viewData([
                                'contactNumber' => '+62811223344',
                            ]),
                        Checkbox::make('have_accepted_offer')
                            ->label(__('filament::widget.form.label.have_accepted_offer'))
                            ->accepted()
                            ->live(onBlur: false),
    ...
    public static function canView(): bool
    {
        ...
        // return $user->hasRole('user') && $user->national_id && $user->mobile_phone_1;
        return $user->national_id && $user->mobile_phone_1;
    }
}
